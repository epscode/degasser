...
class HeatingProfileTableModel extends AbstractTableModel {
	...
	public String getValueAt(int stageNumber, int field) {
		...
		return (String.valueOf(getValueFromHeatingProfile(stageNumber, field) ) );
	}
	*/ 
...
public Integer getValueFromHeatingProfile(int stageNumber, int field) {
	...
	*/
	...
	public void setValueInTable(String value, int stageNumber, int field) {
		...
		// remember this sets the table, not the heating value, somehow get the string array and then set the table value!!

		logger.debug("Setting value at " + stageNumber + "," + field + " to " + value + " (an instance of " + value.getClass() + ")");
		
		/*
		if (stageNumber > getRowCount() ) {
			Vector<String> dataRow= new Vector<String>();
