...
public class SerialCommunications implements SerialPortDataListener {
			...
			if (completeResponse.endsWith("\r\n")) {
				...
				if (waiting) {
					...
					if (statusLine.endsWith("Error") ) {
						...
						logger.info("response is an command error message: " + statusLine);
						// if (errorDialogNeeded) {
						// 	unsolicitedErrorMessages(statusLine);
						// }
						completeResponse = "";
						setError(true); // this was commented out MAYBE FOR A REASON
	...
	public void statusMessages(String statusMessage) {
		...
		for (int messageNumber = 0; messageNumber < messages.length; messageNumber++) {
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					statusPanel.statusTableModel.fireTableDataChanged();
				}
			});
	...
	*/
	...
	public void unsolicitedErrorMessages(String errorMessage) {
		...
		if (errorDialogNeeded) {
			...
			if (message.compareTo("invalidstage") == 0) { 
			...
			} else if (message.compareTo("processingCommandError") == 0) {
				
				NonBlockingMessageDialog errorMessageDialog = new NonBlockingMessageDialog(frame);
				errorMessageDialog.showDialog("Error - Command processing error.",
						"Control Message");
				...
				// JMenuItem powerSupplyMenuItem =  (JMenuItem) menu.getMenu(3).getMenuComponent(5);
				// powerSupplyMenuItem.setText("Turn On Additional Power Supply");
			} else if (errorMessageString.compareTo("commandtimeout") == 0) {

				// estimateFilamentsReceivedError
				JOptionPane.showMessageDialog(frame, "Command Timeout Error", "Error Message",
						JOptionPane.ERROR_MESSAGE);

			} else if (errorMessageString.compareTo("currentOverLimitError") == 0) {

				JOptionPane.showMessageDialog(frame, "Current Over Limit Error", "Error Message",
						JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	boolean toggleRunningMenuItems(boolean state) {
		...
		try {
			// disable editing profile during a run
			 ...
			 menu.getMenu(0).getMenuComponent(8).setEnabled(state);
			 ...
			 System.out.println("Error locking out changes during run " + error);
		 }
